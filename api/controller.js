        ...
        try {
            ...
            new postgres().ExecuteQuery(SQLQuery, function (returnmsg, returnvalue) {
                ...
                if (returnmsg === 'successful') {
                    ...
                    ORetObject.returnvalue = []
                    ORetObject.token = req.headers.authorization
                    ORetObject.status = 200
                    //ORetObject.token =req.headers.A
                    resolve(ORetObject)
                } else {
                    ORetObject.returnmsg = 'DBException'
                    ORetObject.returnvalue = []
                    ORetObject.status = 500
                    resolve(ORetObject)
                }
...
/* LOGOUT */
controller.prototype.logout = function (req, oparameters, callback) {
    var token = ''
    callback(token)
}
